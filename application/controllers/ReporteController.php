...
class ReporteController extends Zend_Controller_Inax {
    ...
    public function semaforostsAction() {
        if (!isset($_SESSION['userInax'])){
            ...
            $this->json(array('color'=>'1d6d02','sesion'=>$sesion,'notificacion'=>''));
        }
    }
    ...
    public function deleteRollAction() {
        $model= new Application_Model_ReporteModel();
        $r=$model->sqlPrepare("delete from dbo.AYT_UsrRollAssigned where id=?",array(filter_input(INPUT_POST,'id')));
        $this->json($r);        
    }
    public function addRollAction() {
        $model= new Application_Model_ReporteModel();
        $r=$model->sqlPrepare("insert into dbo.AYT_UsrRollAssigned(idUsr,idRoll) values (?,?);",array(filter_input(INPUT_POST,'idUsr'),filter_input(INPUT_POST,'idRoll')));
        $this->json($r); 
    }
