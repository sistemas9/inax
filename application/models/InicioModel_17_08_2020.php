...
class Application_Model_InicioModel {
    ...
    public static function getMinimoVenta($item){
        ...
        // $query->execute();
        // $result = $query->fetchAll();
        $result = file_get_contents('minimoVenta.json');
    ...
    public static function getExistencias($item,$sitio,$almacen,$localidad,$documenType,$company) {
        ...
        */
        ...
        foreach($existencias['existencias'] as $Data){
          ...
          if (!$existeCons){
            if (strpos($Data->InventoryWarehouseId, 'CONS') !== false ){
              $existeCons = true;
            }
          }
    ...
    */
        ...
        try {
          ...
          if ($docId == ''){
            ...
            $CURLOPT_POSTFIELDS = "{ 'CurrencyCode': '".$params['CurrencyCode']."'
                                      ...
                                      ,'DeliveryModeCode' : '".$params['DeliveryMode']."'
                                      ,'CustomerPaymentMethodName' : '".$params['PaymMode']."'
                                      ,'SalesTaxGroupCode' : '".((in_array($params['SiteId'],$frontCities))? 'FRONT' : 'VTAS')."'
                                      ,'CustomersReference' : '".$params['comentariosCabecera']."'
                                      ,'DefaultShippingSiteId' : '".$params['SiteId']."'
                                      ,'PaymentTermsName' :'".$params['Payment']."'
    ...
    public function setLineas($params,$idDoc,$tipo,$promoCodeEnable) {
         try {
            // $ws= new Metodos();
            // $ov = $ws->SetLineasDynamics($params,$idDoc,$tipo); 
            // return $ov['result'];
            $resultSet = $this->setLineasEntity($params,$promoCodeEnable);
    ...
    public static function setLineasEntity($params, $promoCodeEnable){
      $token = new Token();
      $flagPatch = true;
      ...
      if (!$flagPatch){
        ...
        if (!$_SESSION['offline']){
          curl_setopt_array(CURL1, array(
              CURLOPT_URL => "https://".DYNAMICS365."/Data/SalesQuotationLines?%24filter=SalesQuotationNumber%20eq%20'".$params[0]['documentId']."'%20and%20ItemNumber%20eq%20'".$params[0]['item']."'&%24select=LineAmount,SalesPrice,RequestedSalesQuantity",
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 30,
        ...
        if($docType != 'CTZN'){
          ...
          if (!$_SESSION['offline']){
            curl_setopt_array(CURL1, array(
                CURLOPT_URL => "https://".DYNAMICS365."/Data/SalesOrderLines?%24filter=SalesOrderNumber%20eq%20'".$params[0]['documentId']."'%20and%20ItemNumber%20eq%20'".$params[0]['item']."'&%24select=LineAmount,SalesPrice,OrderedSalesQuantity",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
    ...
    public static function mandaralerta($param,$cliente) {
      ...
      $adapterDb = Application_Model_InicioModel::$db->getAdapter();
        ...
        print_r($resultadoSinUTF[0]['telefono']);exit();
           ...
           CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
           CURLOPT_CUSTOMREQUEST => "POST",
           CURLOPT_POSTFIELDS => "{\"content\": \"Avance y Tecnología en Plasticos. Se a generado una orden de venta a credito con el número ".$param." a su cuenta ".$cliente." , Si no reconoce esta transaccion comuniquese con su ejecutivo.\", \"to\": [\"".$resultadoSinUTF[0]['telefono']."\",\"6141993515\",\"6141814030\",\"6141151366\"]}",
           CURLOPT_HTTPHEADER => array(
           "accept: application/json",
  ...
  public static function parcharProp($orventa,$proposito,$dlvMode,$tipoPago)
     ...
     * @throws Exception
     */
    static function crearDiario($factura,$journalName,$descripcion,$montoFactura,$diarioCuentaContra,$diarioFPago,$customer,$timbre,$tipoPago,$dlvMode,$referenciap){
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => "{\n\t \t\t\"dataAreaId\": \"".COMPANY."\",\n      \"JournalName\": \"".$journalName."\",\n      \"Description\": \"".$descripcion.", ".$referenciap."\"\n}",
            CURLOPT_HTTPHEADER => array(
            "authorization: Bearer ".$token->getToken()[0]->Token."",
            "content-type: application/json; odata.metadata=minimal",
            "odata-version: 4.0"
...
{ 
    ...
    function sendMail($adress,$asunto,$body) {
        try{
            ...
            $mail->SMTPSecure = 'ssl';
            $mail->SMTPAuth = true;
            $mail->Username = "sistemas6@avanceytec.com.mx";
            $mail->Password = "ZY249VU4076";
  ...
  public static function getSeccion($cliente){
    ...
    curl_setopt_array(CURL1 , array(
      CURLOPT_URL => "https://".DYNAMICS365."/Data/CustomersV3(dataAreaId=%27atp%27,%20CustomerAccount=%27".$cliente."%27)",
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 60,
  ...
  //****funcion para actualizar los clientes que estan en inax con los datos de dynamics
    ...
    public static function getCotPromo($cotId){
      ...
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $now = date('Y-m-d');
      $query = $conn->prepare("SELECT DISTINCT Ov,PromoCode FROM [AYT_PromoRedeemedLog] WHERE Ov = '".$cotId."'");
      $query->execute();
