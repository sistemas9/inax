    ...
    $.ajax({url: "fallasinax/filter-time",dataType: 'json',type: 'POST',data:{f1:f1,f2:f2},
        ...
        success: function (data, textStatus, jqXHR) {
           ...
           if(Number(anterior) !== Number(data.totalError)){
            ...
            $("#totalPeticiones").html(Number(data.total-data.totalError).formatMoney(0, '.', ','));
            ...
            $("#eDiario").html(Number(data.totalDiarioError).formatMoney(0,'.',','));
            $("#eDiarioC").html(Number(data.totalDiarioErrorC).formatMoney(0,'.',','));
            $("#eDiarioE").html(Number(data.totalDiarioErrorE).formatMoney(0,'.',','));
            var errorP=(data.totalPrecio*100)/data.totalWSPrecioComplete;
            var ttt=Number(data.totalWSDiario) + Number(data.totalDiarioError);
            var tttC=Number(data.totalWSDiarioCerrar) + Number(data.totalDiarioErrorC);
            var tttE=Number(data.totalWSDiarioEditar) + Number(data.totalDiarioErrorE);
            var errorEditar=Number((data.totalDiarioError*100/ttt));
            var errorEditarC=Number((data.totalDiarioErrorC*100/tttC));
            var errorEditarE=Number((data.totalDiarioErrorE/tttE)*100);
            $("#ePreciosPor").html(errorP.toFixed(2)+'%');
            $("#eDiarioPor").html(errorEditar.toFixed(2)+'%');
            $("#eDiarioPorC").html(errorEditarC.toFixed(2)+'%');
            $("#eDiarioPorE").html(errorEditarE.toFixed(2)+'%');
            ...
            $("#cDiario").html(Number(data.totalWSDiario).formatMoney(0,'.',','));
            $("#cDiarioC").html(Number(data.totalWSDiarioCerrar).formatMoney(0,'.',','));
            $("#cDiarioE").html(Number(data.totalWSDiarioEditar).formatMoney(0,'.',','));
