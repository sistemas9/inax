...
$(document).on('keydown input change', ".getPrice", function (ev) {
    let promoCodeSelected = $('#promoCode').val() != "None" ? promoCodes[$('#promoCode').val()] : false;
    let getPriceFromWs = false;
    if (edicion == '11111') {
        loaderSwal();
    }
    var bandSrc;
    ...
    if ((this.value != '') && ((ev.type == 'keydown' && (ev.which == 13 || ev.which == 40 || ev.which == 9)) || (bandSrc) || (bandChng))) {
        ...
        if (promoCodeSelected != false && promoCodeSelected.ItemsList[item]) {
            getPriceFromWs = true;
            punitario = 0;
            $("#punitariolinea" + myRow).val(0);
        }
        if (promoCodeSelected == false && lastPromoSelected != "None" && promoCodes[lastPromoSelected].ItemsList[item]) {
            punitario = 0;
            getPriceFromWs = true;
            $("#punitariolinea" + myRow).val(0);
        }
        if (promoCodeSelected == false && edicion == 1 && editionPromoCodeSelected != 'None' && promoCodes[editionPromoCodeSelected].ItemsList[item]) {
            punitario = 0;
            getPriceFromWs = true;
            $("#punitariolinea" + myRow).val(0);
        }
        if (lastPromoSelected != "None" && promoCodes[lastPromoSelected].ItemsList[item]) {
            punitario = 0;
            getPriceFromWs = true;
            $("#punitariolinea" + myRow).val(0);
        }
        ...
        if (eval(punitario) == 0) {
            ...
            if (almacen.indexOf('MRMA') != -1 && flag == true) {
                ...
                if (promoCodeSelected != false && promoCodeSelected.ItemsList[item] && almacen.indexOf('MRMA') == -1 && almacen.indexOf('TRAS') == -1 && almacen.indexOf('EXHB') == -1 && (promoCodeSelected.SiteId = 'All' || promoCodeSelected.SiteId == $("#sitioLineas").val()) && Number(qty) >= Number(promoCodeSelected.ItemsList[item].itemMinQuantity)) {
                    preciocargo = preciocargo - (preciocargo * (promoCodeSelected.ItemsList[item].itemDiscount / 100));
                ...
                $(row).attr('data-preciounit',preciocargo);
                var condiLinea = $('#paytermArt').val();
                if ( $('#pagolineas option:selected').data('paymmode') == '99' && ( condiLinea != 'CONTADO' && condiLinea != 'CONTADO PD' ) ) {
                // if ( $('#pagolineas option:selected').data('paymmode') == '99' ) {
                    revisarLimite('',cliente,tot,'PPD').then(function(res){
                        ...
                        var porcStr = porcentaje.toFixed(2)+'%'
                        $('#limitedecredito').css('width', porcStr );
                        $('#limitedecredito').css('text-align', 'right');
                        $('#limitedecredito').css('color', 'white');
                        $('#limitedecredito').css('background-color', '#26A69A');
                        $('#limitedecredito').css('padding-right', '30px');
                        $('#limitedecredito').html(porcStr);
                        if (porcentaje > 110){
                            $('#limitedecredito').css('text-align', 'left');
                            $('#limitedecredito').css('color', 'white');
                            $('#limitedecredito').css('background-color', 'red');
                            $('#limitedecredito').html('Credito Excedido en 110%');
                        }
                    });
                }else{
                    $('#limitedecredito').removeAttr('style');
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        tmpPunitario = 0;
                        row = $('#cantidad' + myRow).closest('tr');
                        $(row).attr('data-preciounit',data['precioUnitario']);
                        var maxUnitPrice = getMaxUnitPrice();
                        var condiLinea = $('#paytermArt').val();
                        if ( $('#pagolineas option:selected').data('paymmode') == '99' && ( condiLinea != 'CONTADO' && condiLinea != 'CONTADO PD' ) ) {
                        // if ( $('#pagolineas option:selected').data('paymmode') == '99' ) {
...
function getExistencias(item, myRow, unidadNeg, descripcion, excedido, cliente, almacen, type) {
    ...
    $('#modalComplementos #articulo').html('Artículo: ' + item + " " + descripcion);
    ...
    var documenType = $("#DocumentType2").val();
    descripcion = descripcion.replace('”',' PULG');
        ...
        success: function (data) {
            ...
            $('#ExistenciasTodosSitios > tbody').html("");
             if (data.complements.length > 0){
                itemComplements = data.complements;
                // console.log('Lista de complentos', item);
            }
            ...
            for (var i = 0; i < data.datos.length; i++) {
                ...
                if (!(data.datos[i].Almacen.indexOf('AUDI') != -1) && !(data.datos[i].Almacen.indexOf('TRAS') != -1) && !(data.datos[i].Almacen.indexOf('RECM') != -1)) {
                    ...
                    if (data.datos[i].Sitio === sitio) {
                        if (Number(data.datos[i].Existencia) === 0 & data.datos[i].Sitio === 'CHIH' || data.datos[i].Sitio === 'VEQP' & (data.datos[i].Almacen.indexOf('MRMA') != -1 | data.datos[i].Almacen.indexOf('CONS') != -1 | data.datos[i].Almacen.indexOf('ESPC') != -1 | data.datos[i].Almacen.indexOf('EQPS') != -1)) {
                            if (data.datos[i].Almacen.indexOf('MRMA') != -1) {
                                ...
                                if (existenciaCedis <= 0 & existenciaCedisEqps <= 0) {
                                    solicitaTraspaso = '';
                                }
                            }
...
function mandaArticulos(){
     $('.check:checked').each(function(){
        ...
        $('#item'+lineaActual).val($(this).attr('item-idArticulo'));
        $('#descripcion'+lineaActual).val($(this).attr('item-Nombre'));
        $('#cantidad'+lineaActual).val(1);
        $('#cantidad'+lineaActual).attr("data-lastvalue",1);
        $('#unidad'+lineaActual).val('PZA');
        $('#sitio'+lineaActual).val($(this).attr('item-sitio'));
        $('#almacen'+lineaActual).val($(this).attr('item-almacen'));
        $('#localidad'+lineaActual).val('GRAL');
        $('#cantidad'+lineaActual).trigger('change');
    });
    $('#modalComplementos').closeModal();
    }

    $(document).on('click','#enviarComplementos',function(e){ 
    console.log(numObligado);
    console.log(numAlter);
    console.log(compObligado);
    var numObligado = $('.checkobligado:checked').length;
    var numAlter = $('.checkalter:checked').length;
    if ((numObligado > 0 && compObligado == true) || (numAlter >= 0 && (numObligado > 0 && compObligado == true) || ((numObligado >= 0 && compObligado == false)))){
          mandaArticulos();
      }
    else{
        alert("Selecciona minimo un artículo obligado");
    }
})

var compObligado = false;
$(document).on('click','#EnviarExistencias',function(e){
    ...
    $('#tableObligadoComplements').html('');
    $('#tableSugestedComplements').html('');
    if(itemComplements.length > 0){
        console.log('This items has complements');
        var cont = 0;    
        var show = 0; 
        itemComplements.forEach(complement =>{
            switch (complement.complementType) {
                case 'OBLIGADO':
                    complement.datos.forEach(item =>{
                        if(SitioClteEncabezado != item.Sitio) return false;
                         var Code = item.Articulo;                        
                          // console.log("codidgo de articulo",Code);                                                                                      
                        $('#tableObligadoComplements').append(`<tr>
                            <td>${item.Articulo}</td>
                            <td>${complement.complementName}</td>
                            <td>${item.Sitio}</td>
                            <td>${item.Almacen}</td>
                            <td>${item.Existencia}</td>
                            <td><input type="checkbox" id="checkbox${cont}" item-idArticulo="${item.Articulo}" item-Nombre="${complement.complementName}" item-sitio="${item.Sitio}" item-almacen="${item.Almacen}" item-Existencias="${item.Existencia}" class="col l1 s1 m1 check checkobligado"/><label for="checkbox${cont}"></label></td>
                        </tr>`);
                        cont ++;
                        compObligado = true;
                        show = 1;
                ...
                case 'COMPLEMENTO':                  
                    complement.datos.forEach(item => {
                        if(SitioClteEncabezado != item.Sitio) return false;
                        $('#tableSugestedComplements').append(`<tr>
                            <td>${item.Articulo}</td>
                            <td>${complement.complementName}</td>
                            <td>${item.Sitio}</td>
                            <td>${item.Almacen}</td>
                            <td>${item.Existencia}</td>
                            <td><input type="checkbox" id="checkboxob${cont}" item-idArticulo="${item.Articulo}" item-Nombre="${complement.complementName}" item-sitio="${item.Sitio}" item-almacen="${item.Almacen}" item-Existencias="${item.Existencia}" class="col l1 s1 m1 check checkalter"/><label for="checkboxob${cont}"></label></td>
                        </tr>`);
                        cont ++;
                        show = 1;
                    });
 ...
 //    	});
	...
	if (lineasArr !== 'NoResults'){
        ...
        $(lineasTotArr).each(function(){
            ...
            if (this.minimoventa){
                $('#cantidad'+lineaActual).addClass('checkStep');
                $('#cantidad'+lineaActual).attr("data-step",this.minimoventa.MULTIPLEQTY);
            }
...
//deshabilitar funciones para el select de las tarjetas de credito
function disableTJC(unitPrice){
    var opc0 = $('#pagolineas option.TJC')[0];
    var opc1 = $('#pagolineas option.TJC')[1];
    var opc2 = $('#pagolineas option.TJC')[2];
    var opc3 = $('#pagolineas option.TJC')[3];
    var opc4 = $('#pagolineas option.TJC')[4];
    $(opc0).prop('disabled',false);
    $(opc1).prop('disabled',true);
    $(opc2).prop('disabled',true);
    $(opc3).prop('disabled',true);
    $(opc4).prop('disabled',true);
    if (unitPrice >= montos_minimos[0] && unitPrice < montos_minimos[1]){
        $(opc1).prop('disabled',false);    
        $(opc2).prop('disabled',true);
        $(opc3).prop('disabled',true);
        $(opc4).prop('disabled',true);
    }else if (unitPrice >= montos_minimos[1] && unitPrice < montos_minimos[2]){
        $(opc1).prop('disabled',false);    
        $(opc2).prop('disabled',false);
        $(opc3).prop('disabled',true);
        $(opc4).prop('disabled',true);
    }else if (unitPrice >= montos_minimos[2] && unitPrice < montos_minimos[3]){
        $(opc1).prop('disabled',false);    
        $(opc2).prop('disabled',false);
        $(opc3).prop('disabled',false);
        $(opc4).prop('disabled',true);
    }else if (unitPrice >= montos_minimos[3]){
        $(opc1).prop('disabled',false);    
        $(opc2).prop('disabled',false);
        $(opc3).prop('disabled',false);
        $(opc4).prop('disabled',false);
    }
}

///obtener el maximo de los unit prices en rows
function getMaxUnitPrice(){
    //var unitPrices = $('#articulos tbody tr:visible').sort(function(a,b){return +$(b).data('preciounit') - +$(a).data('preciounit')});
    var unitPrices = $('#articulos tbody tr:visible [id*=montocargo]').sort(function(a,b){return +$(b).val().replace('$','').replace(/,/g,'') - +$(a).val().replace('$','').replace(/,/g,'')});
    var element = unitPrices[0];
    //return $(element).data('preciounit');
    if (element){
        elementValue = Number($(element).val().replace('$','').replace(/,/g,''));
        return elementValue;
    }
    return 0;
}

//funcion para revisar limite de credito
function revisarLimite(factura,customer,montocoti,tipopago){
    return new Promise(function(resolve,reject){
        $.ajax({
            url:'inicio/review-Credit-Limit'
            ,type: 'POST'
            ,data: {factura, customer,montocoti,tipopago}
            ,//se revisa si el cliente tiene credito para poder facturar o si es de contado
            beforeSend: function (xhr) {
                Materialize.toast('Actualizando credito consumido.', 3000);
            },
            success: function (data, textStatus, jqXHR) {
                data.montocoti = montocoti;
                resolve(data);
            }
        });
    });
...
//funcion para mandar correo de aviso de servicio
function generarAvisoMail(ov, formData){
    // var fecha_ini = $('#fecha_ini').val();
    // var fecha_fin = $('#fecha_fin').val();
    // var tecnico = $('#tecnico option:selected').text();
    // var tecnico_val = $('#tecnico option:selected').val();
    var fecha_ini = service.fecha_ini;
    var fecha_fin = service.fecha_fin;
    var tecnico = service.tecnico;
    var tecnico_val = service.tecnico_val;
    var factura = service.factura;
    if (fecha_ini != '' && tecnico_val != '0'){
        formData.append('titulo','Peticion de servicio');
        formData.append('mensaje','Estimado {DESTINATARIO} el usuario:');
        formData.append('fecha_ini',fecha_ini);
        formData.append('tecnico',tecnico);
        formData.append('numEmpleado',tecnico_val);
        formData.append('ov',ov);
        formData.append('factura',factura);
        $.ajax({
        url: "inicio/emailservicio",
        type: "POST",
        ...
        success: function (data) {
            data = JSON.parse(data);
            if (data == 'enviado'){
                swal('Exito!.','Aviso generado exitosamente.','success').then(function(res){
                    if(res){
                        $('#calendarModal').closeModal();
                    }
                });
            }else{
                swal('Error!.','Ocurrio un error durante la transaccion.','error');
            }
        }
    });
    }else{
        swal('Atencion!.','Debe especificar una fecha de inicio de servicio tentativa o real y seleccionar un tecnico.','warning');
    }
}